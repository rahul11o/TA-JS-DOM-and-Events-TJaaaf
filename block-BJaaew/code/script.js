...
let root = document.querySelector(".card-container");
let btn_container = document.querySelector(".btn-container");
let inputElem = document.querySelector("header input");

function createCards(ary){
    ary.forEach((element) =>{
...
function handleClick(event){
   ...
   if(elem.name === event.target.innerText){
    root.innerHTML = "";
    createCards(elem.people)
...
function handleKeyup(event){
  got.houses.forEach((elem) =>{
    elem.people.forEach((elem2)=>{
        
        if(elem2.name === event.target.value){
            root.innerHTML = "";
            dummyArr.push(elem2)
            createCards(dummyArr)
        } else if(elem.name === event.target.value){
            root.innerHTML = "";
            createCards(elem.people)
        }
    })
